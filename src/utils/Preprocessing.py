...
from utils.utils_GEAR import *
from utils.utils_FairGNN import *
...
from utils.loader_HNN_GAD import gen_structural_outliers, gen_contextual_outliers, gen_dice_outliers, gen_path_outliers

from utils.CFGT import CFGT
...
def load_data(path_root, dataset):
    ...
    if dataset == 'credit':
        ...
        label_number = 6000
        path_credit = path_root + "/data/dataset/credit"
    ...
    # Load german dataset
        ...
        label_number = 100
        path_german = path_root + "/data/dataset/german"
    ...
    # Load bail dataset
        ...
        predict_attr = "RECID"
        label_number = 100
        path_bail = path_root + "/data/dataset/bail"
        ...
        sens_idx = 0
        label_number = 1000
        path_sythetic = path_root + '/data/dataset/synthetic.mat'
        ...
        test_idx=False
        adj, features, labels, idx_train, idx_val, idx_test, sens, idx_sens_train = load_pokec(dataset,sens_attr,predict_attr,path=path,label_number=label_number,sens_number=sens_number,seed=seed,test_idx=test_idx)
        # 最后一个sens和idx_sens_train好像不太一样，改了一下
...
if __name__ == '__main__':    
    ...
    # info = [adj, features, labels, idx_train, idx_val, idx_test, sens, raw_data_info]
    # edge_index, edge_attr = from_scipy_sparse_matrix(adj)                                                                                
    # data = Data(x=features, edge_index=edge_index, y=labels)
    # data_dice, y_outlier = gen_dice_outliers(data, n = math.ceil(data.num_nodes * 0.05), r_perturb = 0.5, y_outlier=None, random_state=None)
    # data_dice.y = y_outlier
    # print("finish injection dice outliers")
    ...
    # datasets=['synthetic', 'german', 'credit', 'bail', 'pokec_z', 'pokec_n', 'UCSD34']
    datasets=['german', 'credit', 'bail', 'pokec_z', 'pokec_n']
    path_root=''
    for dataset in datasets:
        ...
        path_truecf_data = '../dataset/graphFair_subgraph_cf/'
        generate_cf_true(data, dataset, sens_rate_list, sens_idx, path_truecf_data, save_file=True, raw_data_info=raw_data_info)  # generate
